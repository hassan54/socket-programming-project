...
{   //------If the socket fd is listner socket fd
    if(i==listener)
            {   
                ...
                if(send(newfd,"Hi!You have joined the P2P Network!",strlen("Hi!You have joined the P2P Network!"),0)==-1)
                {
                perror("send");
...
{   
    ...
    int rv;
    if (rv = getaddrinfo(argv[1], argv[2], &hints, &res) != 0) //------calling getaddrinfo with arguments server ip address and port no.
    {
